...
RosPlanner::RosPlanner(const ::ros::NodeHandle &nh,
    ...
    : factory_(), nh_(nh), nh_private_(nh_private), running_(false),
      vis_num_previous_trajectories_(0), vis_completed_count_(0) {
  ...
  nh_private_.param("publish_traversable", p_publish_traversable_,
                    false); // As in voxblox
  ...
  nh_private_.param("max_new_segments", p_max_new_segments_,
...
void RosPlanner::requestNextTrajectory() {
...
void RosPlanner::expandTrajectories() {
...
void RosPlanner::publishTrajectoryVisualization(
  ...
  double max_value =
      (*std::max_element(trajectories.begin(), trajectories.end(),
          ...
          ->value;
  double min_value =
      (*std::min_element(trajectories.begin(), trajectories.end(),
                         TrajectorySegment::comparePtr))
          ->value;
...
bool RosPlanner::checkMinNewValue(
