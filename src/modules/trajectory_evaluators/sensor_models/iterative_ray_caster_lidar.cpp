#include "mav_active_3d_planning/modules/trajectory_evaluators/sensor_models/iterative_ray_caster_lidar.h"
...
namespace mav_active_3d_planning {
    namespace sensor_models {
        ...
        ModuleFactory::Registration <IterativeRayCasterLidar> IterativeRayCasterLidar::registration("IterativeRayCasterLidar");
        ...
        void IterativeRayCasterLidar::setupFromParamMap(Module::ParamMap *param_map) {
            ...
            c_res_x_ = std::min(
                    (int) ceil(p_ray_length_ * p_fov_x_ / ((double) c_voxel_size_ * p_downsampling_factor_)),
                    p_resolution_x_);
            c_res_y_ = std::min(
                    (int) ceil(p_ray_length_ * p_fov_y_ / ((double) c_voxel_size_ * p_downsampling_factor_)),
                    p_resolution_y_);
        ...
        bool
        IterativeRayCasterLidar::getVisibleVoxels(std::vector <Eigen::Vector3d> *result, const Eigen::Vector3d &position,
            ...
            for (int i = 0; i < c_res_x_; ++i) {
                for (int j = 0; j < c_res_y_; ++j) {
                    ...
                    LidarModel::getDirectionVector(&camera_direction, (double) i / ((double) c_res_x_ - 1.0),
        ...
        void IterativeRayCasterLidar::markNeighboringRays(int x, int y, int segment, int value) {
