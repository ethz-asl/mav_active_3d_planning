...
// PruneByValue
ModuleFactoryRegistry::Registration<PruneDirect> PruneDirect::registration("PruneDirect");
...
bool PruneDirect::updateSegment(TrajectorySegment *segment) {
  if (segment->value < p_minimum_value_ || segment->gain < p_minimum_gain_ || segment->cost > p_maximum_cost_ ){
      return false;
  }
