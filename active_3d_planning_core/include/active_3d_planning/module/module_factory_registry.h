#ifndef ACTIVE_3D_PLANNING_CORE_MODULE_FACTORY_REGISTRY_H
#define ACTIVE_3D_PLANNING_CORE_MODULE_FACTORY_REGISTRY_H
...
using ModuleFactoryMethod = std::function<Module *(PlannerI &planner)>;
...
public:
  ...
  // Registration struct
  template <class ModuleType> struct Registration {
    Registration(const std::string &module_name) {
      if (getRegistryMap().find(module_name) == getRegistryMap().end()) {
        ...
        LOG(FATAL) << "ModuleFactory: Cannot register already existing module name '" << module_name << "'!";
      }
    }
  };
...
#endif // ACTIVE_3D_PLANNING_CORE_MODULE_FACTORY_REGISTRY_H
